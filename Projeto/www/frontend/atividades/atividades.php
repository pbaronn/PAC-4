...
<body>
    <div class="fundo">
        ...
        <div class="telas" align="center">
            ...
            <form id="meuFormulario" class="form" method="POST" action="../../backend/processa_formulario.php">
    <div class="borda">    
        <div class="inputs">
            <input type="text" name="nome_atividade" class="nomeatividade" placeholder="Nome da Atividade" required>
<br>
<br>
<!-- Select para profissional -->
            <select name="profissional" class="profissional" required>
                <option value="">Selecione o Profissional</option>
                <?php
// Conectar ao banco de dados
include '../../backend/conecta.php'; // Verifique a conexão aqui
...
$sql = "SELECT cduser, NM_ASSOCIADO FROM user WHERE SN_ADMIN = 1"; // Consulta para profissionais
$result = $conn->query($sql); // Executa a consulta

// Verifique se a consulta foi bem-sucedida e se há resultados
if ($result && $result->num_rows > 0) {
    // Se houver resultados, exibe as opções
    while($row = $result->fetch_assoc()) {
        echo "<option value='" . $row['cduser'] . "'>" . htmlspecialchars($row['NM_ASSOCIADO']) . "</option>";
    }
} else {
    // Caso não haja profissionais ou se a consulta falhou
    echo "<option value=''>Nenhum profissional encontrado</option>";
}

// Não feche a conexão aqui se precisar usá-la depois. Feche no final de todas as operações.
?>
    ...
    <div class="opcoes">
        <a href="../opcoes-atividades/opcoes-atividades.php"><div class="Voltar">Voltar</div></a>
