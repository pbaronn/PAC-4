...
<link rel="stylesheet" type="text/css" href="../vizualizar-associados/vizualizar-associados.css">
                ...
                <div class="inputs">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '<div class="item" data-id="' . $row['cduser'] . '">';
                            echo '<div class="item">' . htmlspecialchars($row['NM_ASSOCIADO']) . '</div>';
                            echo '<button class="deletar" type="button">Deletar</button>';
                            echo '</div>';
                        }
                    } else {
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.deletar').forEach(button => {
        button.addEventListener('click', function () {
            ...
            const userId = itemDiv.dataset.id; // Pega o ID do usuário

            if (confirm('Tem certeza que deseja deletar este usuário?')) {
                // Faz a requisição para deletar o usuário
                fetch('deleta_associados.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ cduser: userId })
                })
